...
async def get_game_by_code(game_code: str, allow_archived: bool = False) -> Game:
    ...
    import aiosqlite
    from game_engine import Game as GameClass, Player, Building, BuildingStatus, parse_building_status
...
def build_enriched_leaderboard(game: Game, game_code: str) -> List[Dict]:
    ...
    global previous_leaderboard, player_capitalization_history, previous_leaderboard_ranks
    ...
    for player in current_leaderboard:
        ...
        unique_key = f"{game_code}:{player_id}"
        history = player_capitalization_history.get(unique_key, {})
        ...
        player_data["growth_round_percent"] = round(growth, 2)

        if unique_key not in player_capitalization_history:
            player_capitalization_history[unique_key] = {
                "previous": player["total_value"],
                "initial": player["total_value"],
            }

        history = player_capitalization_history[unique_key]
...
async def broadcast_update(game_code: str = None):
    ...
    await update_capitalization_history(game_code=game_code)
...
async def update_capitalization_history(game_code: str = None):
    """Обновить историю капитализации всех игроков для конкретной игры"""
    global player_capitalization_history
    
    if not game_code:
        return  # Не можем обновить историю без game_code
    
    try:
        # Получаем игру асинхронно
        game = await get_game_by_code(game_code)
        if not game:
            return
        ...
        # Используем префикс game_code для уникальности игроков в разных играх
        for player_data in leaderboard:
            player_id = player_data["player_id"]
            total_value = player_data["total_value"]
            
            # Создаем уникальный ключ: game_code:player_id
            unique_key = f"{game_code}:{player_id}"
            ...
            if unique_key not in player_capitalization_history:
                ...
                player_capitalization_history[unique_key]["previous"] = total_value
    except Exception as e:
        api_logger.error(f"Ошибка обновления истории капитализации для {game_code}: {e}", exc_info=True)
...
async def get_player_state(
    ...
    try:
        ...
        try:
            ...
            # Используем уникальный ключ с game_code для изоляции игр
            unique_key = f"{game_code}:{player_id}"
            ...
            if history.get("last_round", 0) < current_round:
                history["previous"] = history.get("current", total_value)
                history["last_round"] = current_round
            ...
            api_logger.warning(f"Ошибка расчета приростов в get_player_state: {e}")
            growth_round = 0
            prev_capitalization = total_value
            initial_capitalization = total_value
...
async def get_miniapp_leaderboard(game_code: str = Query(..., description="Код игры (6 цифр)")):
    ...
    for idx, player_data in enumerate(leaderboard):
        ...
        if player:
            ...
            # Используем уникальный ключ с game_code для изоляции игр
            total_value = player_data["total_value"]
            unique_key = f"{game_code}:{player_data['player_id']}"
            history = player_capitalization_history.get(unique_key, {})
